...
void Generate_dataset(float *h_input, size_t dim_x, size_t dim_y, int nBatches, int offset){
	for(int b=0; b<nBatches; b++){
		for(size_t d=0; d<dim_y; d++){
			for(size_t s=0; s<dim_x; s++){
				h_input[b*dim_x*dim_y + d*dim_x + s] = (rand() / (float)RAND_MAX)*((float) (b+1));
				if(s>(dim_x-offset)) h_input[b*dim_x*dim_y + d*dim_x + s] = 10000;
			}
		}
	}
}
...
int main(int argc, char* argv[]) {
	...
	if (argc==6) {
		...
		nBatches  = strtol(argv[3],&pEnd,10);
		offset    = strtol(argv[4],&pEnd,10);
		device_id = strtol(argv[5],&pEnd,10);
	}
	else {
		printf("Argument error!\n");
		printf(" 1) x dimension\n");
		printf(" 2) y dimension\n");
		printf(" 3) Number of batches\n");
		printf(" 4) offset\n");
		printf(" 5) device id\n");
        return(1);
	}
	...
	srand(time(NULL));
	Generate_dataset(h_input, dim_x, dim_y, nBatches, offset);
	...
	cudaMalloc((void **) &d_MSD,   MSD_size);
	cudaMalloc((void **) &d_MSD_nElements, MSD_elements_size);
	...
	for(int b=0; b<nBatches; b++){
		MSD_Kahan(&h_input[b*dim_x*dim_y], dim_x, dim_y, offset, &signal_mean, &signal_sd);
