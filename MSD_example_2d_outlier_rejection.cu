...
void Generate_dataset_outlier_rejection(float *h_input, size_t dim_y, size_t dim_x, int offset, float percent, float sigma){
	for(size_t d=0; d<dim_y; d++){
		for(size_t s=0; s<dim_x; s++){
			h_input[d*dim_x + s]=rand() / (float)RAND_MAX;
			if(s>(dim_x-offset)) h_input[d*dim_x + s]=10000;
		}
	}
	
	long int nSpikes = ( ((float) dim_y)*((float) dim_x))*percent;
	for(long int f=0; f<nSpikes; f++){
		...
		if(x<dim_x && y<dim_y){
			h_input[y*dim_x + x]=0.2885*sigma;
		}
	}
}

void Generate_dataset_outlier_rejection_box(float *h_input, size_t dim_y, size_t dim_x, int offset, float percent, float sigma){
	for(size_t d=0; d<dim_y; d++){
		for(size_t s=0; s<dim_x; s++){
			h_input[d*dim_x + s]=rand() / (float)RAND_MAX;
			if(s>(dim_x-offset)) h_input[d*dim_x + s]=10000;
	...
	long int nSpike_rows = floor(((float) dim_y)*percent);
	for(long int f=0; f<nSpike_rows; f++){
		for(size_t x=0; x<dim_x; x++){
			h_input[f*dim_x + x]=0.2885*sigma;
		}
	}
}
...
int main(int argc, char* argv[]) {
	...
	char * pEnd;
	if (argc==6) {
		dim_y         = strtol(argv[1],&pEnd,10); // this with CONV_SIZE gives signal size
		...
		printf(" 1) number of DM trials\n");
		printf(" 2) number of time samples per DM trial\n");
		printf(" 3) offset\n");
		printf(" 4) device id\n");
	...
	srand(time(NULL));
	Generate_dataset_outlier_rejection(h_input, dim_y, dim_x, offset, 0.05, 7.0);
